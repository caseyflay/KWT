...
```{r kruskal.test for association of SNPs from the A2 population with SNPs filtered on heterozygous alleles in the tolerant father}
...
A2all_long <- A2all_long[T.16_01_01A_Dad2 >= 6] #only include snps which have a heterozygous parent at that site (dad is tolerant)
...
```{r kruskal.test for association of SNPs from the A2 family with SNPs filtered on heterozygous alleles in the susceptible mother to check if any tolerance is inherited from mum}
## change parent to filter the heterozygotes on line 7.
A2all_long <- fread("KWT_12E_run1_PS1.csv")
A2all_long <- setnames(A2all_long, c(names(A2all_long)), c(str_replace_all(names(A2all_long), ":", "_") ))
A2all_long <- setnames(A2all_long, c(names(A2all_long)), c(str_replace_all(names(A2all_long), "-", "_") ))
...
A2all_long <- A2all_long[CK51_06_MumA >= 6] #only include snps which have a het mum at that site (mum is thought to be susceptible)
...
y <- melt.data.table(y, id.vars = 1)
y <- y[, plant := NULL]
z <- y[value<0.001] #only include those with a high p value
Kruskal_p.value <- as.numeric(z$value)
snp_candidate <- select(A2all_wide, z$variable)
snp_candidate_sites <- names(snp_candidate)
...
qplot(z$variable, Kruskal_p.value)#, aes(x="SNPposition", y="Kruskal.test_p-value") 
```
```{r kruskal.test for association of genotypes from the A1 and A2 population with phenotype}
